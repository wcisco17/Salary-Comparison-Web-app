...
public class GlassdoorTest implements IScraper<Glassdoor> {
    Glassdoor website;
    ...
    @Override
    public void setUp() {
        website = new Glassdoor();
        companies = ExcecuteDB.getAllCompanies();
    }

    @Test
    @DisplayName("Should have the right url, name from DB - (Glassdoor)")
    @Override
    public void shouldCheckWesbiteNotNull() throws IOException {
        ...
        Assertions.assertEquals(website.title, SiteType.GLASSDOOR);
    ...
    public void shouldCreateJob() throws IOException {
        Job job = new Job("Glassdoor Test company name", "test job title", "$100,000 - $300,000", "logo.png",
